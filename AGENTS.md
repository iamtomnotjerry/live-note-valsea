...
- Chỉ rõ file/route (`src/app/[locale]/...`, `src/features/...`); nêu Server vs Client; nếu đổi copy → cập nhật **mọi** file `messages/*.json` có locale đang dùng (hoặc chạy lại script gen SEA sau khi sửa `en.json`).
